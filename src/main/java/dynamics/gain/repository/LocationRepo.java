...
public class LocationRepo {
    ...
    public boolean update(Location location) {
        String sql = "update locations set name = ?, location_uri = ?, description = ?, needs = ?, town_id = ?, count = ? where id = ?";
        jdbcTemplate.update(sql, new Object[] { location.getName(), location.getLocationUri(), location.getDescription(), location.getNeeds(), location.getTownId(), location.getCount(), location.getId() });
        return true;
    }
